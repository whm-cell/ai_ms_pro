...
当前判断：harness 可用；stage-00 转向 bounded 增量；`AGENTS.md` 保持短触发层；context budget / line density 是 strict gate；runtime artifacts 默认 local-only；active validation 仍只认 WS-01 / WS-02；外部能力、prototype、code-shape、config、mock-data/data-activation、reuse/retirement、run metrics 和 enterprise claim 均按 bounded evidence / boundary 处理。
...
- 最新 changelog：[Borrowed Harness Controls](./changelog/2026-06-21-borrowed-harness-controls.md)
