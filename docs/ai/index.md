...
当前判断：harness 可用；stopped burn-in session 已进入 closeout / split 阶段；context budget 是 strict gate；WS-01 是 capability validation sample；WS-04 浏览器 MVP 已完成 smoke；Godot、正式素材、音频、移动端和发布流水线仍是后续独立范围。
...
- Game/static：`check_threejs_snake_contract.py`，其他游戏 smoke 按当前 workstream/status 选择。
