...
- 说明：本 handoff 是 harness control-plane 收尾；当前 active validation 只使用 WS-01 / WS-02，精确要求仍以 `docs/requirements/*` 为准。
...
- Validation boundary：WS-01 Three.js Snake 与 WS-02 Harness Trace Console 是当前 active browser validation surface
...
- Success Criteria：canonical handoff、working-context、index 和 status 保持一致；closeout checks 通过或只剩已知 warnings；分批 review 顺序明确。
...
- Burn-in readiness：5 个 ready-for-upgrade-discussion gap 已有 keep-advisory 决策；其余主要是 future real-event sample lane。
...
- Workflow simple-skip：WS-01 pause/resume 与 reset-best 已提供 2 个 accepted real workflow-task samples；`GAP-WORKFLOW-SIMPLE-SKIP` 已记录 keep-advisory，后续需要 WS-01 之外的 simple-skip、negative 或 process-tax 样本。
