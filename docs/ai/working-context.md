...
- REQDOC-003 已完成首轮标准化，绑定 REQ-007 / REQ-008 / REQ-009 与 WS-03，并以 `apps/godot-platformer-slice/` 验证 repo-native 首轮玩法切片；完整 Godot 工程仍是 proposed / 待确认。
...
4. REQDOC-003 后续若继续推进，应先决定是否新建真实 Godot engine spike；不要把完整游戏工程直接塞进 root repo 默认面。
5. 后续真实 PRD / 非平凡功能任务要继续登记 with/without eval；`prd-to-project-skills` 与 `progressive-feature-development` 当前已有 SAMPLE-001，仍只是 1/2，不足以升级 always-on。
6. 后续真实多人 / 多 AI PR 要用 `$team-pr-conflict-control` 记录 touch-set overlap 和 coordination action，先观察是否值得升级更多阻断策略。
7. 下一次 stage compression 继续清理完成型 handoff，避免 Stage-00 历史进入长期默认面。
...
- Candidate workflow skill eval 当前只有 SAMPLE-001 accepted，两个 skills 仍是 1/2；不得为了升级 always-on 伪造样本或把简单任务拖入完整流程。
...
- context budget 已收紧为 80/90 高水位、ADR 到达预算、stage status 行数 warning；当前默认面已压回 90% 以下，但仍超过 80%。
...
- Candidate workflow skills 当前均为 1/2 accepted samples，不得升级 always-on。
