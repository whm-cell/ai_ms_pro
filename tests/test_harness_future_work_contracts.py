...
class HarnessFutureWorkContractsTest(unittest.TestCase):
    def test_repository_contracts_cover_future_work_gaps_with_approved_sampling_paths(self) -> None:
        ...
        self.assertEqual(2, report.approved_for_sampling_count)
        self.assertEqual(0, report.blocked_until_adr_count)
        ...
        self.assertTrue(states["GAP-TRACE-REMOTE-INTEROP"].sample_collection_allowed)
        self.assertFalse(states["GAP-TRACE-REMOTE-INTEROP"].missing_adr_refs)
        ...
        self.assertIn("Allowed by contract record", states["GAP-TRACE-REMOTE-INTEROP"].sample_collection_boundary)
        self.assertTrue(states["GAP-AGENTIC-CASCADE-STOP"].sample_collection_allowed)
        self.assertFalse(states["GAP-AGENTIC-CASCADE-STOP"].missing_adr_refs)
        self.assertIn("Allowed by contract record", states["GAP-AGENTIC-CASCADE-STOP"].sample_collection_boundary)
    ...
    def test_approved_sampling_requires_adopted_adr_with_contract_coverage(self) -> None:
        ...
        self.assertTrue(any("must mention gap_id GAP-TRACE-REMOTE-INTEROP" in error for error in report.errors))
        self.assertTrue(any("must mention contract id FWC-test-remote" in error for error in report.errors))
        self.assertTrue(any("must cover auth_model" in error for error in report.errors))
    ...
    def test_cli_json_output_is_stable(self) -> None:
        ...
        self.assertIn('"approved_for_sampling_count": 2', result.stdout)
