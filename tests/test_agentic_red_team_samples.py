...
class AgenticRedTeamSamplesTest(unittest.TestCase):
    ...
    def test_repository_samples_pass(self) -> None:
        ...
        self.assertGreaterEqual(report.accepted_replay_or_real_count, 8)
        self.assertGreaterEqual(report.accepted_real_incident_count, 2)
        self.assertGreaterEqual(report.accepted_real_by_risk.get("sandbox-claim-honesty", 0), 2)
        self.assertEqual(check_agentic_red_team_samples.REQUIRED_RISKS, check_agentic_red_team_samples.RISKS)
        for risk in check_agentic_red_team_samples.REQUIRED_RISKS:
            self.assertGreaterEqual(report.accepted_by_risk.get(risk, 0), 1)
