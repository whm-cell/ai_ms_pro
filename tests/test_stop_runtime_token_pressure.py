...
class StopRuntimeTokenPressureTest(unittest.TestCase):
    def test_emits_bounded_additional_context_without_raw_output(self) -> None:
        ...
        self.assertIn("Large tool output", context)
        self.assertIn("line 6", context)
        self.assertIn("Fresh input/cache miss spike", context)
    ...
    def test_limits_warning_count(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            root = Path(tempdir)
            write_config(root)
            transcript = root / "rollout.jsonl"
        ...
        warning_lines = [line for line in context.splitlines() if line.startswith("- ")]
        self.assertLessEqual(len(warning_lines), stop_runtime_token_pressure.MAX_WARNINGS)
        self.assertIn("more warning(s) omitted", context)
