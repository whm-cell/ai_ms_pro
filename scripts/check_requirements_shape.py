...
VERIFICATION_METHOD_RE = re.compile(r"\bverification method\b|验证方式|验证方法|验收方式|验收方法|测试方式|测试方法|验证命令|测试命令|\b(smoke|test|pytest|go test|pnpm test|npm test)\b", re.IGNORECASE)
...
def build_report() -> RequirementShapeReport:
    errors: list[str] = []
    warnings: list[str] = []
    source_docs = parse_doc_ids(iter_docs(SOURCE_DIR), ("文档编号：", "文档编号:"), REQDOC_RE, "REQDOC", errors)
    ...
    check_technical_assumption_lines(
        [*source_docs.values(), *req_docs.values(), *ws_docs.values()],
        warnings,
    )
