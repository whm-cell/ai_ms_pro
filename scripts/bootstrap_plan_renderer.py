...
def render_plan(project_name: str, stage_label: str) -> str:
    ...
    sections = [
        render_plan_header(),
        render_usage_boundary(),
        render_project_goals(project_name),
        render_scope_definition(),
        render_business_and_modules(),
        render_stage_plan(),
        render_architecture_decisions(),
        render_risks_and_constraints(),
        render_document_governance(),
    ]
    return "\n\n".join(sections) + "\n"


def render_plan_header() -> str:
...
def render_project_goals(project_name: str) -> str:
    return textwrap.dedent(
        f"""\
        ## 项目目标
...
def render_scope_definition() -> str:
    return textwrap.dedent(
        """\
        ## 范围定义
...
def render_business_and_modules() -> str:
    return textwrap.dedent(
        """\
        ## 业务线索与模块划分
...
def render_architecture_decisions() -> str:
    return textwrap.dedent(
        """\
        ## 技术与架构决策
...
def render_risks_and_constraints() -> str:
    return textwrap.dedent(
        """\
        ## 风险与约束
...
def render_document_governance() -> str:
    return textwrap.dedent(
        """\
        ## 文档治理约定
