...
DEFAULT_SAMPLES = ROOT / "docs" / "ai" / "standards" / "pre-tool-use-preflight-samples.jsonl"
ID_RE = re.compile(r"^PRE-SAMPLE-\d{4}-\d{2}-\d{2}-[a-z0-9]+(?:-[a-z0-9]+)*$")
DATE_RE = re.compile(r"^\d{4}-\d{2}-\d{2}$")
SOURCE_TYPES = {"real-tool-call", "synthetic-regression", "manual-review"}
HOOK_RESULTS = {"warned", "silent"}
...
class PreflightSampleReport:
    sample_path: str
    record_count: int
    real_sample_count: int
    accepted_real_sample_count: int
    accepted_real_warning_sample_count: int
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate PreToolUse preflight burn-in samples.")
    parser.add_argument("--samples", default=str(DEFAULT_SAMPLES), help="Preflight sample JSONL path.")
...
def build_report(path: Path = DEFAULT_SAMPLES) -> PreflightSampleReport:
    errors: list[str] = []
    warnings: list[str] = []
    ...
    for line_no, record in records:
        ...
        hook_result = text(record.get("hook_result"))
        findings = text_list(record.get("triggered_findings"))
        if source_type == "real-tool-call":
            real_count += 1
        if source_type == "real-tool-call" and outcome == "accepted":
            accepted_real_count += 1
            if hook_result == "warned" and findings != ["none"]:
                accepted_warning_count += 1
        if record.get("false_positive") is True:
            false_positive_count += 1
    if accepted_warning_count == 0:
        warnings.append("no accepted real preflight warning sample recorded yet")
    return PreflightSampleReport(
        relative(path),
        len(records),
        real_count,
        accepted_real_count,
...
def validate_record(
    ...
    if text(record.get("schema_version")) != "pre-tool-use-preflight-sample/v1":
        errors.append(f"{prefix}: schema_version must be pre-tool-use-preflight-sample/v1")
    validate_date(record, "sampled_at", prefix, errors)
    validate_choice(record, "source_type", SOURCE_TYPES, prefix, errors)
    validate_choice(record, "hook_result", HOOK_RESULTS, prefix, errors)
    ...
    validate_bounded_required_text(record, "risk_summary", prefix, errors)
    ...
    validate_list_choices(record, "operator_decisions", DECISIONS, prefix, errors)
...
def validate_outcome_rules(
    ...
    hook_result = text(record.get("hook_result"))
    findings = text_list(record.get("triggered_findings"))
    decisions = text_list(record.get("operator_decisions"))
    action_taken = text_list(record.get("action_taken"))
    evidence_refs = text_list(record.get("evidence_refs"))
    if hook_result == "silent" and findings != ["none"]:
        errors.append(f"{prefix}: silent samples must use triggered_findings=['none']")
    if hook_result == "warned" and findings == ["none"]:
        errors.append(f"{prefix}: warned samples need at least one finding")
    ...
    if outcome == "accepted" and hook_result == "warned" and decisions == ["none"]:
        errors.append(f"{prefix}: accepted warning samples need operator_decisions")
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            if key_text in FORBIDDEN_KEYS:
                errors.append(f"{prefix}: forbidden raw tool key: {key_text}")
...
def emit_text(report: PreflightSampleReport) -> None:
    print("PreToolUse preflight sample audit:")
    ...
    print(f"- accepted real warning samples: {report.accepted_real_warning_sample_count}")
