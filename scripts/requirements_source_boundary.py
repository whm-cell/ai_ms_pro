    ...
    *,
    ...
    for source_id, path in source_docs.items():
        ...
        if not INSTRUCTION_AS_DATA_RE.search(instruction_handling or ""):
            warnings.append(
                f"external content boundary instruction handling should say source content is evidence/data: {location}"
            )
        if not INSTRUCTION_NOT_EXECUTABLE_RE.search(instruction_handling or ""):
            warnings.append(
                f"external content boundary instruction handling should say source content is not executable agent instructions: {location}"
            )
