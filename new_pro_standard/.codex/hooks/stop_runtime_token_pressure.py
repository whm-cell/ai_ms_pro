...
from build_runtime_compression_draft import write_runtime_compression_draft  # noqa: E402
...
def build_additional_context(payload: dict[str, Any], root: Path = ROOT) -> str:
    ...
    draft_path = try_write_draft(transcript_path, transcript, config, root)
    lines = [
        "Runtime token pressure detected; wrote a Runtime compression draft.",
        f"Transcript: `{display_transcript_path(transcript_path, root)}`",
        f"Draft: `{display_transcript_path(draft_path, root)}`" if draft_path else "Draft: `<not written; inspect transcript manually>`",
        "Next turn: read the runtime-only recovery draft as evidence, then manually promote durable conclusions if needed.",
    ]
...
def try_write_draft(
    ...
    try:
        return write_runtime_compression_draft(transcript_path, transcript, config, root=root)
    except Exception:
        return None
...
def display_transcript_path(path: Path, root: Path = ROOT) -> str:
    try:
