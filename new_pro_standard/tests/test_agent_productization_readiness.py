...
class AgentProductizationReadinessTest(unittest.TestCase):
    def test_template_records_are_valid_but_deferred(self) -> None:
        report = readiness.build_report()

        self.assertEqual(report.errors, [])
        self.assertEqual(report.capability_count, 12)
        self.assertIn("starter-template", report.targets)
        self.assertIn("APR-01", report.mvp_capabilities)
        self.assertIn("APR-12", report.mature_capabilities)
        self.assertEqual(12, len(report.review_findings))
        self.assertEqual(report.status_counts.get("deferred"), 12)
        self.assertNotIn("covered", report.status_counts)
