...
class HarnessCollectionConfigTest(unittest.TestCase):
    def test_repository_collection_config_is_valid(self) -> None:
        ...
        self.assertGreaterEqual(report.active_capture_gate_count, 8)
        self.assertGreaterEqual(report.real_sample_capture_gate_count, 7)
        self.assertEqual(report.real_sample_area_count, 6)
        self.assertEqual(report.real_sample_priority_count, 3)
        self.assertEqual(report.real_sample_ledger_action_count, 1)
        self.assertEqual(report.real_sample_readiness_count, 1)
        self.assertFalse(
    ...
    def test_future_work_gap_requires_explicit_trigger(self) -> None:
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "TRIGGERS: future-work gap needs explicit contract-boundary trigger: GAP-TRACE-REMOTE-INTEROP",
            report.errors,
        )
    ...
    def test_active_capture_gate_missing_from_choices_is_reported(self) -> None:
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "CAPTURE_GATES: active capture gate missing from choices: requires-real-event",
            report.errors,
        )
    ...
    def test_real_sample_capture_gate_requires_focused_command_package(self) -> None:
        missing_command = (
            ...
            "--capture-gate requires-real-event --capture-card"
        )
        commands = tuple(
            command
        ...
        self.assertIn(
            ...
            f"requires-real-event -> {missing_command}",
            report.errors,
        )
    ...
    def test_real_sample_priority_requires_capture_focus_choice(self) -> None:
        ...
        self.assertFalse(report.ok)
        self.assertIn(
            "CAPTURE_FOCUS_PRIORITIES: active real-sample priority missing from choices: P2",
            report.errors,
        )
