...
class CheckBurnInLedgerTest(unittest.TestCase):
    def test_repository_ledger_matches_blocking_candidates(self) -> None:
        ...
        self.assertEqual([], result.upgrade_eligible_checks)
        self.assertEqual([], result.upgrade_review_needed_checks)
        self.assertIn("check_code_shape.py", result.checks_needing_samples)
        self.assertIn("check_tool_contracts.py", result.checks_needing_samples)
        code_shape_row = next(row for row in result.rows if row.check == "check_code_shape.py")
        self.assertEqual(0, code_shape_row.accepted_samples)
        self.assertEqual(2, code_shape_row.remaining_samples)
        self.assertFalse(code_shape_row.upgrade_review_needed)
        self.assertEqual([], code_shape_row.evidence_refs)
        tool_contract_row = next(row for row in result.rows if row.check == "check_tool_contracts.py")
        self.assertEqual(0, tool_contract_row.accepted_samples)
        self.assertEqual(2, tool_contract_row.remaining_samples)
        self.assertFalse(tool_contract_row.upgrade_review_needed)
        self.assertEqual([], tool_contract_row.evidence_refs)
