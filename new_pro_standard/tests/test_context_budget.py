...
""",
    ...
    def test_main_blocks_by_default_but_warning_only_exits_zero(self) -> None:
        report = check_context_budget.ContextBudgetReport(
            default_surface_tokens=91,
        ...
        with (
            patch.object(sys, "argv", ["check_context_budget.py"]),
            patch.object(check_context_budget, "build_report", return_value=report),
            redirect_stdout(StringIO()),
        ):
            self.assertEqual(check_context_budget.main(), 1)

        with (
            patch.object(sys, "argv", ["check_context_budget.py", "--warning-only"]),
            patch.object(check_context_budget, "build_report", return_value=report),
            redirect_stdout(StringIO()),
        ):
