...
VALID_ACTIVATION_STATES = {"smoke", "shadow-real", "retired"}
...
def validate_manifest_row(
    ...
    findings.extend(validate_manifest_activation_fields(manifest, line_no, row))
...
def validate_manifest_enums(manifest: str, line_no: int, row: dict[str, object]) -> list[MockDataFinding]:
    ...
    activation_state = string_field(row, "activation_state")
    if activation_state and activation_state not in VALID_ACTIVATION_STATES:
        allowed = ", ".join(sorted(VALID_ACTIVATION_STATES))
        findings.append(
            manifest_finding(
                manifest,
                line_no,
                "invalid-scenario-manifest-row",
                f"`activation_state` must be one of {allowed}",
            )
        )
    return findings
...
def validate_manifest_activation_fields(
    manifest: str,
    line_no: int,
    row: dict[str, object],
) -> list[MockDataFinding]:
    findings: list[MockDataFinding] = []
    for field in ("real_adapter_path", "retire_when"):
        value = row.get(field)
        if value is not None and not string_field(row, field):
            findings.append(manifest_finding(manifest, line_no, "invalid-scenario-manifest-row", f"`{field}` must be a string"))
    value = row.get("activation_evidence_refs")
    if value is not None and not string_list_field(row, "activation_evidence_refs"):
        findings.append(
            manifest_finding(
                manifest,
                line_no,
                "invalid-scenario-manifest-row",
                "`activation_evidence_refs` must be a non-empty list of strings when present",
            )
        )
    return findings
