...
PREFLIGHT_CATALOG = HOOKS_DIR / "preflight_risk_catalog.py"
...
def literal_codes_from_paths(paths: tuple[Path, ...], call_name: str) -> set[str]:
    codes: set[str] = set()
    for path in paths:
        if path.exists():
            codes.update(literal_codes(path, call_name))
    return codes
...
def audit() -> AlignmentReport:
    ...
    preflight_emitted = literal_codes_from_paths((PREFLIGHT_HOOK, PREFLIGHT_CATALOG), "PreflightFinding")
