...
class UpgradeDecisionQueueContext:
    ledger_action: str = ""
    readiness: str = ""
    source_metric: str = ""
...
def expected_upgrade_decision_context(gap_id: str, errors: list[str]) -> UpgradeDecisionQueueContext:
    ...
    if not items:
        errors.append(explain_non_upgrade_decision_lane(gap_id))
        return UpgradeDecisionQueueContext()
    if len(items) > 1:
        errors.append(f"{gap_id}: expected one review-upgrade-decision item, found {len(items)}")
        return UpgradeDecisionQueueContext()
    item = items[0]
...
def explain_non_upgrade_decision_lane(gap_id: str) -> str:
    ...
    if not current_items:
        return f"{gap_id}: no current collection queue item found for review-upgrade-decision lane"
    ...
    return (
        ...
        "upgrade decision review requires review-upgrade-decision"
    )
